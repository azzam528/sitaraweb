...
export function useDashboardView() {
  ...
  const patientStats = computed(() => [
    ...
    {
      label: 'Risiko / TB RO',
      value: String(summary.value.tb_ro_patients ?? summary.value.high_risk_patients ?? 0),
  ...
  const getTrendValue = (item) => {
    ...
    return item?.value ?? item?.adherence ?? item?.medication_adherence ?? item?.percentage ?? 0
  }
      ...
      if (patRes.data && patRes.data.length > 0) {
        ...
        monitoredPatients.value = patients.slice(0, 4).map((p, idx) => {
          ...
          return {
            ...
            reason: p.clinical_note ? p.clinical_note.slice(0, 45) + '...' : (isIntensive ? 'Fase Intensif - Kepatuhan Obat' : 'Fase Lanjutan - Kontrol Rutin'),
