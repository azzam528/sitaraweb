  ...
  <div class="medicine-list-page">
    ...
    <header class="page-header">
      <div class="header-content">
        ...
        <p class="page-subtitle">
          Kelola permintaan obat dan pengambilan obat pasien TB.
        </p>
      </div>
    </header>
    ...
    <div class="card table-card">
      <div class="card-header-tabs flex-between">
        <h2 class="card-title">Permintaan Obat Pasien</h2>
            ...
            <tr v-else v-for="req in paginatedRequests" :key="req.id">
                ...
                <div class="patient-info">
                  ...
                  <div class="patient-meta">
                    <span class="patient-name font-semibold">
                      {{
                        req.treatment?.patient?.full_name ||
                        "Pasien #" + req.treatment_id
                      }}
                    </span>
              ...
              <td>
                <div class="med-requested">
                  <span
                    :class="['type-pill', getTypeClass(req.medicine?.category)]"
                  >
                    {{ req.medicine?.name || "OAT" }}
                  </span>
                  <span class="font-bold text-dark ml-2"
                    >{{ req.quantity }} {{ req.medicine?.unit || "Tab" }}</span
                  >
                </div>
              </td>
              <td>
      ...
      <div class="pagination-section">
        <div class="pagination-info">
          ...
          - {{ Math.min(currentPage * pageSize, filteredRequests.length) }} dari
    ...
    <div class="card distribution-card">
      <div class="card-header">
        <h2 class="card-title">Riwayat Penyaluran Obat</h2>
      </div>
        ...
        <table class="data-table custom-table">
          ...
          <tbody>
            <tr v-if="distributionHistory.length === 0">
              <td colspan="5" class="text-center py-6 text-muted">
                Belum ada riwayat distribusi obat yang telah disetujui atau
                ditolak.
              </td>
            </tr>
            <tr v-else v-for="hist in distributionHistory" :key="hist.id">
              ...
              <td>
                <span class="font-semibold text-dark">{{
                  hist.treatment?.patient?.full_name ||
                  "Pasien #" + hist.treatment_id
                }}</span>
                <span class="text-xs text-muted block"
                  >NIK: {{ hist.treatment?.patient?.nik || "-" }}</span
                >
              </td>
              <td>
                <span
                  :class="['type-pill', getTypeClass(hist.medicine?.category)]"
                >
                  {{ hist.medicine?.name || "OAT" }}
                </span>
              </td>
              <td>
                <span class="font-bold">{{ hist.quantity }}</span>
                {{ hist.medicine?.unit || "Tablet" }}
              </td>
              <td>
