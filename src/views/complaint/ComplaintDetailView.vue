        ...
        <div class="col-main">
          ...
          <div class="card">
            ...
            <div class="complaint-quote-box">
              <p class="complaint-quote">
                "{{
                  complaint.description ||
                  "Tidak ada deskripsi keluhan tertulis."
                }}"
              </p>
            </div>
              ...
              <div class="detail-row">
                ...
                <span class="detail-val">
                  <span
                    ...
                    :class="'severity-' + (complaint.severity || 'mild')"
                  >
            ...
            <div v-if="complaint.response" class="response-history-box">
              <div class="response-header-row mb-1">
                ...
                <span class="status-badge status-resolved text-xs"
                  >Telah Ditanggapi</span
                >
              </div>
              <p class="response-text">{{ complaint.response }}</p>
              <span
                class="response-meta"
                v-if="complaint.responded_at || complaint.updated_at"
              >
                Diperbarui:
                {{
                  formatDate(complaint.responded_at || complaint.updated_at)
                }}
                ({{
                  formatTime(complaint.responded_at || complaint.updated_at)
                }}
                WIB)
              </span>
            </div>
              ...
              <h3 class="side-title">
                {{
                  complaint.status === "resolved" || complaint.response
                    ? "Perbarui Tanggapan Medis"
                    : "Beri Tanggapan Medis"
                }}
              </h3>
              <span
                class="status-badge"
                :class="'status-' + (complaint.status || 'pending')"
              >
                {{ formatStatus(complaint.status) }}
              </span>
            </div>
            ...
            <form @submit.prevent="submitUpdateStatus">
              <div class="form-group mb-3">
                ...
                <p class="text-xs text-muted mt-2">
                  * Laporan keluhan akan otomatis berstatus
                  <strong>Selesai</strong> setelah tanggapan disimpan.
                </p>
              </div>
